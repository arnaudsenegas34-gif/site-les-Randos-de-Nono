...
<section class="newsletter-band" aria-labelledby="newsletter-heading">
  <div class="newsletter-inner">
    ...
    <?php
    // Le double opt-in ajoute deux états : « on t'a envoyé un lien » après la
    // saisie, et « c'est confirmé » après le clic. La checklist n'est proposée
    // qu'au second — c'est le clic qui vaut consentement.
    ?>
    <?php if ( 'confirme' === $rando_nono_nl_status ) : ?>
      <p class="newsletter-msg newsletter-msg-ok">
        Presque&nbsp;! Un e-mail vient de partir vers ta boîte : clique sur le lien qu'il contient
        pour confirmer ton inscription. Pense à regarder dans les indésirables.
      </p>
    <?php elseif ( 'confirme_ok' === $rando_nono_nl_status || 'ok' === $rando_nono_nl_status ) : ?>
      <p class="newsletter-msg newsletter-msg-ok">
    ...
    <?php elseif ( 'lien_invalide' === $rando_nono_nl_status ) : ?>
      <p class="newsletter-msg newsletter-msg-error">
        Ce lien de confirmation n'est plus valable. Réinscris-toi ci-dessous, un nouveau lien partira aussitôt.
      </p>
    <?php elseif ( 'desabonne' === $rando_nono_nl_status ) : ?>
      <p class="newsletter-msg newsletter-msg-ok">Tu as bien été désabonné. Ton adresse a été supprimée de la liste.</p>
    ...
    <?php else : ?>
      <form class="newsletter-form" method="post" action="<?php echo esc_url( home_url( add_query_arg( null, null ) ) ); ?>">
        ...
        <div class="contact-honeypot" aria-hidden="true">
          <label for="site_web_nl" aria-hidden="true">Site web</label>
          <input type="text" id="site_web_nl" name="site_web_nl" tabindex="-1" autocomplete="off" aria-hidden="true">
        </div>
        <input type="email" id="newsletter_email" name="newsletter_email" autocomplete="email" aria-label="Adresse e-mail" placeholder="ton@email.fr" required>
        <button type="submit" name="rando_nono_newsletter_submit" value="1">S'abonner</button>
            <p class="form-legal">
          Ton adresse sert uniquement à t'envoyer les nouvelles randonnées. Un lien de
          désinscription figure dans chaque e-mail, et ton adresse est alors supprimée —
...
<footer class="site-footer">
  ...
  <nav class="footer-nav" aria-label="Liens rapides">
    ...
    <a href="<?php echo esc_url( get_post_type_archive_link( 'randonnee' ) ); ?>">Toutes les randos</a>
  ...
  <div class="footer-col footer-legal">
    ...
    <a class="social-instagram" href="https://www.instagram.com/a._.sng?igsh=MWpyYWVyazh6NWJ6dw==" target="_blank" rel="noopener noreferrer" aria-label="Suivre Nono sur Instagram">
